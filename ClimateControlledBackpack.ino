...
void loop() {
  ...
  toggleFans(true); // true = On, false = Off
...
void toggleHeaters(bool onOff) {
  digitalWrite(heaterPin, onOff);
}

void toggleFans(bool onOff) {
...
void notif_warning() {
  ...
  delay(200);
  noTone(speakerPin);
  // delay(200); // no delay b/c the delay will be added in the alarm() method --> This delay also serves to blink screen
}

void notif_monitorMode() {
  tone(speakerPin, Bb6_);
  delay(100);
  noTone(speakerPin);

  tone(speakerPin, Ab6_);
  delay(100);
  noTone(speakerPin);

  tone(speakerPin, Gb6_);
  delay(100);
  noTone(speakerPin);

  tone(speakerPin, Db6_);
  delay(150);
  noTone(speakerPin);
  delay(50);
}